"""Market price memory — record scouted prices OVER TIME so arbitrage works.

A market only shows live prices while a ship is parked there, so at any instant
only ~1 market in a system is "lit". Spatial arbitrage needs two markets lit at
once (a good's buy price at one + sell price at another) — which almost never
happens with a couple of roving probes. So we PERSIST every observation: each time
a ship visits a market we record its prices, and the trade-route finder runs over
the recorded map (built up as probes sweep), not just the one currently-lit market.
...
def _conn():
    ...
    c.execute("CREATE TABLE IF NOT EXISTS prices ("
              "waypoint TEXT, good TEXT, system TEXT, buy INTEGER, sell INTEGER, ts REAL, "
    ...
    try:
        ...
        for g in trade_goods:
            ...
            c.execute("INSERT OR REPLACE INTO prices VALUES (?,?,?,?,?,?)",
                      (waypoint, g["symbol"], system, bp, sp, now))
        c.commit()
...
def price_map(system: str, max_age: float = 3600.0) -> list[dict]:
    """The recorded markets for a system as best_arbitrage input — latest prices per
    waypoint, dropping observations older than ``max_age`` seconds (prices drift)."""
    try:
        c = _conn()
        cutoff = time.time() - max_age
        rows = c.execute(
            "SELECT waypoint, good, buy, sell FROM prices WHERE system=? AND ts>=?",
            (system, cutoff)).fetchall()
    except Exception:  # noqa: BLE001
        return []
    by_wp: dict[str, list] = {}
    for wp, good, buy, sell in rows:
        by_wp.setdefault(wp, []).append({"symbol": good, "purchasePrice": buy, "sellPrice": sell})
